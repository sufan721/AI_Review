...
```mermaid
%%{init: {"themeVariables": {"fontSize": "12px"}, "flowchart": {"nodeSpacing": 20, "rankSpacing": 26, "wrappingWidth": 120, "padding": 5}}}%%
flowchart LR
  U[注册用户]
  D[DeepSeek API]
  Q[千问百炼 API]
...
```mermaid
%%{init: {"themeVariables": {"fontSize": "12px"}, "sequence": {"wrap": true, "wrapPadding": 6, "actorMargin": 8, "width": 62, "boxMargin": 6, "messageMargin": 20, "noteMargin": 8, "mirrorActors": false, "diagramMarginX": 4, "diagramMarginY": 6, "bottomMarginAdj": 4, "actorFontSize": 12, "messageFontSize": 11, "noteFontSize": 11}}}%%
sequenceDiagram
  actor U as 用户
...
```mermaid
%%{init: {"themeVariables": {"fontSize": "11px"}, "flowchart": {"nodeSpacing": 4, "rankSpacing": 18, "wrappingWidth": 46, "padding": 3}}}%%
flowchart TB
...
| `note` | `id`、`user_id`、`title`、`content`、`content_version`、`index_status` | 用户创建的 Markdown 笔记。 |
| `document` | `id`、`user_id`、`file_name`、`content`、`content_version`、`index_status` | 用户上传的 Markdown 文件。 |
| `knowledge_base` | `id`、`user_id`、`name` | 用户私有知识库。 |
| `knowledge_base_item` | `knowledge_base_id`、`resource_type`、`resource_id` | 知识库与笔记/文件的多对多关系。 |
| `document_chunk` | `id`、`document_type`、`document_id`、`content_version`、`position_start`、`position_end` | MySQL 中的切片定位与索引管理记录。 |
...
erDiagram
  ...
  DOCUMENT["资料文件"]
  KNOWLEDGE_BASE["知识库"]
  KNOWLEDGE_BASE_ITEM["知识库条目"]
  DOCUMENT_CHUNK["文档切片"]
  USER ||--o{ NOTE : 创建
  USER ||--o{ DOCUMENT : 上传
  USER ||--o{ KNOWLEDGE_BASE : 拥有
  KNOWLEDGE_BASE ||--o{ KNOWLEDGE_BASE_ITEM : 包含
  NOTE ||--o{ DOCUMENT_CHUNK : 切片
  DOCUMENT ||--o{ DOCUMENT_CHUNK : 切片
```
...
erDiagram
  ...
  CHAT_SESSION ||--o{ CHAT_MESSAGE : 含
  CHAT_MESSAGE ||--o{ MESSAGE_CITATION : 引用
  DOCUMENT_CHUNK ||--o{ MESSAGE_CITATION : 被引
```
